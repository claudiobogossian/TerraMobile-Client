...
public class PhotoActivity extends Activity implements SurfaceHolder.Callback {
	...
	private boolean storeByteImage(Context mContext, byte[] imageData,
		...
		try {
			...
			Bitmap bitmapImage = BitmapFactory.decodeByteArray(imageData, 0,
					imageData.length, options);
			bitmapImage.compress(CompressFormat.JPEG, quality,
	...
	public void surfaceCreated(SurfaceHolder holder) {
		try {
			...
			cameraParameters
					.setFocusMode(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO); // set
																					// auto-focus
