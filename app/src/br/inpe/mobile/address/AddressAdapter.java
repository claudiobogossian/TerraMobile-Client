...
public class AddressAdapter extends CursorAdapter implements Filterable {
	...
	public void bindView(View v, Context ctx, Cursor c) {
		...
		txt_number
				.setText("Número: " + c.getString(c.getColumnIndex("number")));

		TextView txt_lote = (TextView) v.findViewById(R.id.item_lote);
		txt_lote.setText("Lote: "
				+ Utility.correctNull(c.getString(c.getColumnIndex("featureId")))
				+ "\n");
	}
	...
	public static Cursor getAddressCursor(String propertieFilter)
			throws SQLException {
		...
		Dao<Address, Integer> addressDao = DatabaseHelper.getDatabase()
				.getAddressDao();
		...
		QueryBuilder<Address, Integer> addressQueryBuilder = addressDao
				.queryBuilder();
		...
		if (propertieFilter != null) {
			addressQueryBuilder.where().like("name", propertieFilter).or()
