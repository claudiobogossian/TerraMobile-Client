...
public class Photo extends Activity implements SurfaceHolder.Callback {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// "Erro de configura̤̣o no aplicativo!"));
		// finish();
		// }
		...
		bt_cancel.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				File file = new File(pathfullapp, getPhotoName());
				file.delete();
		...
		bt_ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				setResult(RESULT_OK,
						new Intent().putExtra("RESULT", getPhotoName()));
				finish();
			}
		});
	}
	...
	private void setPhotoName(String name) {
	...
	Camera.PictureCallback jpegCallback = new Camera.PictureCallback() {
		...
		public void onPictureTaken(byte[] _data, Camera _camera) {
			if (_data != null) {
				if (StoreByteImage(Photo.this, _data, 90)) {
					confirmPicture();
				} else {
					setResult(RESULT_CANCELED, new Intent().putExtra("RESULT",
							"Erro ao salvar foto!"));
